...
inline Vect3d Sphere(GLfloat t) {
	const float radius = 0.5f;
	const float rot = 15.f;
	float x = t * 2 * radius - radius;
	float r = sqrt(radius * radius - x * x);
	return Vect3d(x, r * sin(rot * M_PI * t), r * cos(rot * M_PI * t));
}

inline Vect3d Gaussian(GLfloat t) {
	const float mu = 0.f;
	const float sigma = 0.5f;
	const float rot = 15.f;
	float x = 2 * t - 1.0f;
	float r = exp(-0.5f * pow(((x - mu) / sigma), 2)) * 1 / (sigma * sqrt(2 * M_PI));
	return Vect3d(x, r * sin(rot * M_PI * t), r * cos(rot * M_PI * t));
}
//This fills the <vector> *a with data. 
void CreateCurve(vector <Vect3d> *a, int n, int c = 1)
{
	GLfloat step=1.f/n;
	switch (c)
	{
	...
	case 2:
		for (int i = 0; i < n; i++)
		{
			a->push_back(Sphere(i * step));
		}
		break;
	case 3:
		for (int i = 0; i < n; i++)
		{
			a->push_back(Gaussian(i * step));
		}
		break;
	default:
		break;
